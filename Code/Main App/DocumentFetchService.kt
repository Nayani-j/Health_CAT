...
class DocumentFetchService() : Service() {
    ...
    companion object {
        private const val TAG = "DocumentFetchService"
        private const val DOCUMENT_PATH = "medicines/vttOx9SNXc9lnkX7SViD"
    ...
    private fun fetchNotification() {
        ...
        firestore.document(DOCUMENT_PATH)
            ...
            .addOnSuccessListener { document ->
                if (document.exists()) {
                    val medicine = document.getString("mm")
                    val quantity = document.getLong("mq")?.toInt() ?: 0



                    if (!medicine.isNullOrBlank()) {
                        showNotification(medicine + "   " + quantity)
                    } else {
